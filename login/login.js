...
function fxEnsureRunning(fx, minDuration) {
    ...
    function frame(t) {
        ...
        for (let i = fx.flashes.length - 1; i >= 0; i--) {
            ...
            ctx.globalAlpha = Math.pow(1 - age / f.life, 2);
...
function goToRealMain() {
    try {
        sessionStorage.setItem('wof_arrival', JSON.stringify({
            ...
            rank: (currentUser && currentUser.rank) || 'user'
        }));
    } catch (e) {}
...
function initLoginForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        if (username.length < 3 || username.length > 24) {
            errorEl.textContent = getNestedValue(translations, 'login.error_username') || 'Username: 3-24 characters';
            return;
        }
        if (password.length < 8) {
            errorEl.textContent = getNestedValue(translations, 'login.error_password') || 'Password: minimum 8 characters';
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                btn.disabled = false;
                btn.textContent = getNestedValue(translations, 'login.login_btn') || 'LOGIN';
            ...
            errorEl.textContent = err.name === 'AbortError'
                ...
                : (getNestedValue(translations, 'login.error_server') || 'Server connection error');
            btn.disabled = false;
            btn.textContent = getNestedValue(translations, 'login.login_btn') || 'LOGIN';
        }
    });
...
function initOTPButtons() {
    ...
    if (verifyBtn) {
        verifyBtn.addEventListener('click', async () => {
            ...
            if (code.length !== 6) {
                errorEl.textContent = getNestedValue(translations, 'verify.error_code') || 'Please enter the complete 6-digit code!';
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    verifyBtn.textContent = getNestedValue(translations, 'verify.verify_btn') || 'VERIFY';
    ...
    if (resendLink) {
        resendLink.addEventListener('click', async (e) => {
            ...
            try {
                ...
                alert(err.name === 'AbortError' ? 'Server not responding' : 'Server connection error');
            }
        });
    }
}
